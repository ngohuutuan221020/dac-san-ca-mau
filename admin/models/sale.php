...
class Product extends Db
{
    ...
    function getAllProductsByPageSale($page, $perPage)
    {
        // Tính số thứ tự trang bắt đầu  
        $firstLink = ($page - 1) * $perPage;
        //Dùng LIMIT để giới hạn số lượng hiển thị 1 trang 
    ...
    function addNewProduct($name, $manu_id, $type_id, $price, $pro_image, $description,$soluong,$nsx, $hsd, $feature, $created_at)
    {
        $sql = self::$connection->prepare("INSERT INTO `products`(`name`, `manu_id`, `type_id`, `price`, `pro_image`, `description`, `soluong`,`nsx`,`hsd`, `feature`, `created_at`)
        VALUES ('$name',$manu_id,$type_id, ? ,'$pro_image',' $description',' $soluong',' $nsx',' $hsd',$feature,'$created_at')");
        $sql->bind_param("i", $price);
        $sql->execute(); //return an object
    }
    function updateProduct($name, $manu_id, $type_id, $price, $pro_image, $description,$soluong, $nsx, $hsd,$feature, $created_at,$id){
        $query = self::$connection->prepare("UPDATE products SET name = ?,manu_id = ?,type_id = ?,
        price = ?,pro_image = ?,description = ?,soluong = ?,nsx = ?,hsd = ?,feature = ?,created_at=? WHERE id = ? ");
        $query->bind_param("siiissisi",$name,$manu_id,$type_id,$price,$pro_image,$description,$soluong, $nsx, $hsd,$feature,$created_at,$id);
